        ...
        if ch == '(':
            ...
            if depth == 0 and start is not None:
                ...
                if bd and bd.lower() not in ('null', 'none', '0', '0000-00-00', '00-00-0000', 'n/a', '-', ''):
                    # Check if it was resolved in previous script
                    from scripts.test_smart_dob import clean_dob_string
                    dob, reason = clean_dob_string(birthdate, cols[13], age)
                    if not dob:
                        print(f"Unresolved: id={sql_id}, name='{name}', raw_bd='{birthdate}', age='{age}'")
