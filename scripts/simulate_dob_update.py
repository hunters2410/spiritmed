...
def main():
    ...
    for sql_p in sql_patients:
        ...
        # Only update if current DOB is null/empty
        if not cur_dob or cur_dob.lower() in ('null', 'none', ''):
            ...
            planned_updates.append({
                ...
                'calculated_age': get_age(target_dob),
                'reason': sql_p['reason'],
                'raw_bd': sql_p['raw_birthdate'],
                'raw_age': sql_p['raw_age']
            })

    print(f'\nTotal Supabase patients ready to receive date_of_birth: {len(planned_updates)}')
    
    direct_dob = [u for u in planned_updates if not u['reason'].startswith('inferred')]
    inferred_dob = [u for u in planned_updates if u['reason'].startswith('inferred')]

    print(f'  - From parsed legacy birthdate strings (e.g. 1982-02-01): {len(direct_dob)}')
    print(f'  - From legacy age values (e.g. 72 -> 1951-01-01): {len(inferred_dob)}')

    print('\nSample Direct Birthdate Updates (first 10):')
    for u in direct_dob[:10]:
        print(f"  {u['name']} (PNO: {u['pno']}): NULL -> {u['target_dob']} (Age: {u['calculated_age']}, raw: '{u['raw_bd']}')")

    print('\nSample Inferred from Age Updates (first 10):')
    for u in inferred_dob[:10]:
        print(f"  {u['name']} (PNO: {u['pno']}): NULL -> {u['target_dob']} (Age: {u['calculated_age']}, raw_age: '{u['raw_age']}')")

    current_with_dob = sum(1 for p in sp_patients if p.get('date_of_birth'))
    print(f'\nCurrently with DOB in Supabase: {current_with_dob} / {len(sp_patients)} ({current_with_dob/len(sp_patients)*100:.1f}%)')
    print(f'After update with DOB in Supabase: {current_with_dob + len(planned_updates)} / {len(sp_patients)} ({((current_with_dob + len(planned_updates))/len(sp_patients))*100:.1f}%)')
