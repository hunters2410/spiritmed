...
# Delete test rows
for pn in ["99999","99998"]:
    urllib.request.urlopen(urllib.request.Request(
        SUPABASE_URL+f"/rest/v1/patients?patient_number=eq.{pn}",
        headers={**H,"Prefer":"return=minimal"}, method="DELETE"), timeout=10)
...
for line in lines:
    ...
    # fee_idnumber -> id_passport_number (col 26)
...
# Insert batches of 200
print(f"Inserting in batches of 200 ...")
ok=0; err=0; errors=[]
    ...
    try:
        ...
        for p in batch:
            ...
            try:
                ...
                err+=1
                errors.append(f"  {p['patient_number']}: {e2.code} - {e2.read().decode()[:80]}")
            time.sleep(0.02)
    except Exception as ex:
        err+=len(batch); errors.append(f"Batch {i//200+1}: {ex}")

    if (i//200+1)%5==0:
        print(f"  {min(i+200,len(patients))}/{len(patients)} ({min(i+200,len(patients))/len(patients)*100:.0f}%) OK={ok} Err={err}",flush=True)
    time.sleep(0.02)
