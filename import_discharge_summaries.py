...
DISCHARGE_FILE = r"C:\Users\Acer P16\Documents\Spiritmed\hospital update\database 4\discharge.sql"
...
def create_patient_record(patient_name, phone, address, old_pid):
    pid = str(uuid.uuid5(BASE_UUID, f"patient_created_for_discharge:{old_pid or patient_name}"))
...
def main():
    print("Starting Clean Discharge Summaries Import (Stripping <p>, <br>, etc.)...")
    ...
    with open(DISCHARGE_FILE, 'r', encoding='utf-8', errors='ignore') as f:
        content = f.read()

    rows = extract_all_rows_robust(content, 'discharge')
    print(f"Found {len(rows)} total discharge rows in discharge.sql")

    discharges_to_insert = []
    skipped_count = 0
    ...
    for r in rows:
        ...
        history = cols[15] # medical_history / reason_for_admission
        treatment = cols[16] # treatment_done / treatment_summary
        diagnosis = cols[17] # diagnosis_text / discharge_diagnosis
        followupplan = cols[18] # follow_up_plan / follow_up_instructions
        ...
        diagnosis_clean = clean_html(diagnosis)
        followup_clean = clean_html(followupplan)
        ...
        discharge_uuid = str(uuid.uuid5(BASE_UUID, f"discharge_summary_old_id:{old_discharge_id}"))
        ...
        discharge_obj = {
            ...
            "discharge_date": created_at_iso,
            "reason_for_admission": history_clean,
            "treatment_summary": treatment_clean,
            "discharge_diagnosis": diagnosis_clean,
            "follow_up_instructions": followup_clean,
            "recipient": recipient_val.strip(),
            "diagnosis_text": diagnosis_clean,
            "medical_history": history_clean,
            "treatment_done": treatment_clean,
            "follow_up_plan": followup_clean,
            "report_date": report_date_str,
            "created_at": created_at_iso
        }

        discharges_to_insert.append(discharge_obj)

    print(f"\nPrepared {len(discharges_to_insert)} cleaned discharge summaries for upsert.")
    ...
    for i in range(0, len(discharges_to_insert), batch_size):
        batch = discharges_to_insert[i:i+batch_size]
        payload = json.dumps(batch).encode('utf-8')
        
        req = urllib.request.Request(
            f"{SUPABASE_URL}/rest/v1/discharge_summaries",
            data=payload,
            headers={
                "apikey": SERVICE_ROLE_KEY,
                "Authorization": f"Bearer {SERVICE_ROLE_KEY}",
        ...
        try:
            with urllib.request.urlopen(req) as resp:
                ...
                print(f"Upserted batch {i//batch_size + 1}: {len(batch)} records (Total: {inserted_total}/{len(discharges_to_insert)})")
        except urllib.error.HTTPError as e:
            print(f"Error upserting batch starting at index {i}:", e.read().decode('utf-8'))

    # Clean any remaining HTML tags in existing discharge_summaries database records
    print("\nCleaning any remaining HTML tags in existing discharge_summaries database records...")
    req_all = urllib.request.Request(
        f"{SUPABASE_URL}/rest/v1/discharge_summaries?select=id,medical_history,treatment_done,discharge_diagnosis,follow_up_plan",
        headers={"apikey": SERVICE_ROLE_KEY, "Authorization": f"Bearer {SERVICE_ROLE_KEY}"}
    )
    with urllib.request.urlopen(req_all) as resp:
        all_dis = json.loads(resp.read().decode('utf-8'))

    cleaned_db_count = 0
    for dis in all_dis:
        patch_payload = {}
        for field in ['medical_history', 'treatment_done', 'discharge_diagnosis', 'follow_up_plan']:
            val = dis.get(field)
            if val and ('<' in val and '>' in val or '&' in val):
                patch_payload[field] = clean_html(val)
        if patch_payload:
            req_up = urllib.request.Request(
                f"{SUPABASE_URL}/rest/v1/discharge_summaries?id=eq.{dis['id']}",
            ...
            try:
                ...
                print(f"Error patching record {dis['id']}:", e)

    print(f"Cleaned {cleaned_db_count} additional records directly in Supabase.")
    print(f"\nSUCCESS! All discharge summaries imported cleanly without <p>, <br>, etc.")
