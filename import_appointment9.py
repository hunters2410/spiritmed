...
def main():
    ...
    # Load old patient id mapping from patient (8).sql if available
    old_id_to_pn = {}
    ...
    for t in app_tuples:
        ...
        mapped_type = map_appointment_type(remarks)
    ...
    print(f"Matched {matched_count} appointments out of {len(app_tuples)} (Unmatched: {unmatched_count})")
    print("Appointment Types to Import:")
    for k, v in type_counts.most_common():
        print(f"  {k}: {v}")
    print("Statuses to Import:")
    for k, v in status_counts.most_common():
        print(f"  {k}: {v}")

    # Perform deletion and batch insert
    print("\nClearing appointments table in Supabase...", flush=True)
    del_r = requests.delete(f"{SUPABASE_URL}/rest/v1/appointments?id=neq.00000000-0000-0000-0000-000000000000", headers=HEADERS, timeout=30)
    print("Clear response status:", del_r.status_code)

    print(f"Batch inserting {len(appointments_to_import)} appointments...", flush=True)
    batch_size = 500
    inserted = 0
    for i in range(0, len(appointments_to_import), batch_size):
        batch = appointments_to_import[i:i+batch_size]
        ins_r = requests.post(f"{SUPABASE_URL}/rest/v1/appointments", headers=HEADERS, json=batch, timeout=60)
        if ins_r.status_code in (200, 201):
            inserted += len(batch)
            print(f"Inserted {inserted}/{len(appointments_to_import)}...")
        else:
            print(f"Batch insert error: {ins_r.status_code} - {ins_r.text[:200]}")

    print(f"\nFinished! Successfully imported {inserted} appointments.")
