"""
import_appointments6.py
======================
Imports and updates all 17,216 appointment records from database/appointment (6).sql
into Supabase public.appointments table with exact historical status mapping:
...
DEFAULT_DOCTOR_UUID = "90a905bc-d22a-4db3-bd43-2c1c6bf488e0"
...
def main():
    print("=== Step 1: Loading registered patients & doctors from Supabase ===", flush=True)
    all_patients = []
    from_idx = 0
    page_size = 1000
    while True:
    ...
    # Load old patient_id mapping from patient (5).sql
    old_id_to_pn = {}
    ...
    # ── Step 2: Parse appointment (6).sql ─────────────────────────────────────
    ...
    print(f"\n=== Appointment Import & Status Analysis ===")
    print(f"  Total Appointments Parsed : {len(appointments_to_import)}")
    print(f"  Matched to Patients       : {matched_count} / {len(appointments_to_import)} ({(matched_count/len(appointments_to_import))*100:.1f}%)")
    ...
    # ── Step 3: Bulk Upsert into Supabase public.appointments ─────────────────
    print("\n=== Step 3: Upserting Appointments into Supabase ===", flush=True)
    bulk_upsert("appointments", appointments_to_import)

    print("\n=== ALL 17,216 APPOINTMENTS IMPORTED & STATUSES UPDATED SUCCESSFULLY! ===", flush=True)
