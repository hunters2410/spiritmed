...
payload = json.dumps([{
    ...
    "background_history": "Test background history",
...
try:
    with urllib.request.urlopen(req) as resp:
        ...
        print("REFERRAL INSERT TEST SUCCESS:", res)
    
    # Delete test record
    req_del = urllib.request.Request(
        f"{SUPABASE_URL}/rest/v1/referral_forms?id=eq.{test_id}",
        headers={"apikey": SERVICE_ROLE_KEY, "Authorization": f"Bearer {SERVICE_ROLE_KEY}"},
        method="DELETE"
    )
    with urllib.request.urlopen(req_del) as resp:
        print("REFERRAL DELETE TEST CLEANUP SUCCESS")
except urllib.error.HTTPError as e:
    print("REFERRAL INSERT TEST ERROR:", e.read().decode('utf-8'))
