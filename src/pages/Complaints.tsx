...
import {
    Plus, Search, MessageSquare, Pencil, Trash2, X,
    ChevronLeft, ChevronRight, Activity
} from 'lucide-react';

interface Complaint { id: string; name: string; description: string; created_at: string; }

const inputCls = "w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg outline-none focus:ring-2 focus:ring-purple-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm";
...
export function Complaints() {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        let res;
        if (editing) {
            res = await supabase.from('complaints').update(form).eq('id', editing.id);
        } else {
            res = await supabase.from('complaints').insert([{ ...form, branch_id: profile.branch_id }]);
        }
        if (res.error) {
            setError(res.error.code === '23505' ? 'This complaint already exists.' : res.error.message);
        } else {
            setShowModal(false);
            loadData();
        }
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                        <MessageSquare className="w-8 h-8 text-purple-600" /> Complaints
                    </h1>
                    <p className="text-gray-600 dark:text-gray-400 mt-1">Manage the list of patient chief complaints</p>
                </div>
                <button onClick={openAdd}
                    className="flex items-center gap-2 bg-purple-600 text-white px-4 py-2 rounded-lg hover:bg-purple-700 transition shadow-md font-semibold">
                    <Plus className="w-5 h-5" /> Add Complaint
                </button>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4">
                <div className="relative max-w-md">
                    ...
                    <input type="text" placeholder="Search complaints..." value={search}
                        ...
                        className="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-purple-500 outline-none bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm" />
                </div>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm clinical-table border-collapse border border-gray-200 dark:border-gray-700">
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                            {loading ? (
                                <tr><td colSpan={4} className="px-6 py-10 text-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-purple-600 mx-auto" /></td></tr>
                            ) : paginated.length === 0 ? (
                                <tr><td colSpan={4} className="px-6 py-10 text-center text-gray-500">No complaints found. Click "Add Complaint" to create one.</td></tr>
                            ) : paginated.map((c, idx) => (
                                <tr key={c.id} className="hover:bg-purple-50/30 dark:hover:bg-purple-900/10 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-gray-500 dark:text-gray-400">{c.description || '—'}</td>
                                    <td className="px-6 py-4">
                                        <div className="flex justify-center gap-2">
                                            <button onClick={() => openEdit(c)}
                                                className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-100 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400 rounded-lg text-xs font-bold hover:bg-purple-200 transition">
                        ...
                        {totalPages > 1 && (
                            <div className="flex gap-2">
                                ...
                                {[...Array(totalPages)].map((_, i) => (
                                    <button key={i} onClick={() => setCurrentPage(i + 1)} className={`w-8 h-8 rounded-lg text-xs font-bold transition ${currentPage === i + 1 ? 'bg-purple-600 text-white' : 'border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:bg-white dark:hover:bg-gray-700'}`}>{i + 1}</button>
                                ))}
            ...
            {showModal && (
                <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[60] p-4">
                    <div className="bg-white dark:bg-gray-800 rounded-xl w-full max-w-md shadow-2xl">
                        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-purple-50 dark:bg-purple-900/20 rounded-t-xl">
                            <h2 className="text-lg font-bold text-gray-900 dark:text-white">{editing ? 'Edit Complaint' : 'Add Complaint'}</h2>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <input required value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} className={inputCls} placeholder="e.g. Headache" autoFocus />
                            </div>
                            <div>
                            ...
                            <div className="flex gap-3 pt-2">
                                ...
                                <button type="submit" disabled={submitting} className="flex-1 py-2.5 bg-purple-600 text-white rounded-lg text-sm font-bold hover:bg-purple-700 transition disabled:opacity-50">{submitting ? 'Saving...' : (editing ? 'Update' : 'Add Complaint')}</button>
                            </div>
                        </form>
                    </div>
                </div>
