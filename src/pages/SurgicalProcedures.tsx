...
export function SurgicalProcedures() {
    ...
    const [dataList, setDataList] = useState<SurgicalProcedure[]>([]);
    const [loading, setLoading] = useState(true);
    const [search, setSearch] = useState('');
    const [showModal, setShowModal] = useState(false);
    const [editing, setEditing] = useState<SurgicalProcedure | null>(null);
    ...
    function openEdit(item: SurgicalProcedure) {
        setEditing(item);
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        if (res.error) {
            setError(res.error.code === '23505' ? 'This procedure already exists.' : res.error.message);
        } else {
            setShowModal(false);
            loadData();
        }
    ...
    async function handleDelete(id: string) {
        ...
        await supabase.from('surgical_procedures').delete().eq('id', id);
        loadData();
    }
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                        <Activity className="w-8 h-8 text-blue-600" /> Surgical Procedures
                    </h1>
                    <p className="text-gray-600 dark:text-gray-400 mt-1">Manage the master list of surgical procedures</p>
                </div>
                    ...
                    <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                            {loading ? (
                                ...
                                <tr><td colSpan={4} className="px-6 py-10 text-center text-gray-500">No procedures found.</td></tr>
                                ...
                                <tr key={item.id} className="hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-gray-500 dark:text-gray-400">{item.description || '—'}</td>
                ...
                <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[60] p-4">
                    <div className="bg-white dark:bg-gray-800 rounded-xl w-full max-w-md shadow-2xl">
                        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-blue-50 dark:bg-blue-900/20 rounded-t-xl">
                            <h2 className="text-lg font-bold text-gray-900 dark:text-white">{editing ? 'Edit Procedure' : 'Add Procedure'}</h2>
                            <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600"><X className="w-5 h-5" /></button>
                        </div>
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            {error && <div className="p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 rounded-lg flex items-center gap-2 text-red-600 text-xs font-bold"><X className="w-4 h-4" />{error}</div>}
                            <div>
                                <label className={labelCls}>Procedure Name</label>
                                <input required value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} className={inputCls} placeholder="e.g. Appendectomy" autoFocus />
                            </div>
                            <div>
                                <label className={labelCls}>Description (Optional)</label>
                                <div className="relative">
                                    <FileText className="absolute left-3 top-3 w-4 h-4 text-gray-400" />
                                    <textarea rows={3} value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} className={`${inputCls} pl-10 pt-2`} placeholder="Describe the procedure details..." />
                                </div>
                            </div>
                            <div className="flex gap-3 pt-2">
                                ...
                                <button type="submit" disabled={submitting} className="flex-1 py-2.5 bg-blue-600 text-white rounded-lg text-sm font-bold hover:bg-blue-700 transition disabled:opacity-50">{submitting ? 'Saving...' : (editing ? 'Update' : 'Add Procedure')}</button>
                            </div>
                        </form>
                    </div>
                </div>
