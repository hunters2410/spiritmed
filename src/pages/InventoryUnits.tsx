...
import { Plus, Search, Ruler, Pencil, Trash2, X, ChevronLeft, ChevronRight, Activity } from 'lucide-react';
...
export function InventoryUnits() {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        let res;
        if (editingUnit) {
            res = await supabase.from('inventory_units').update(payload).eq('id', editingUnit.id);
        } else {
            res = await supabase.from('inventory_units').insert([payload]);
        }
    ...
    async function handleDelete(id: string) {
        if (!confirm('Are you sure you want to delete this unit? This might affect existing inventory items.')) return;
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                        <Ruler className="w-8 h-8 text-indigo-600" /> Measurement Units
                    </h1>
                    <p className="text-gray-600 dark:text-gray-400 mt-1">Define units of measure (e.g. Vial, Tablet, Boxes, ML)</p>
                </div>
                <button onClick={() => { setEditingUnit(null); setFormData({ name: '', description: '' }); setError(null); setShowModal(true); }}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4">
                <div className="relative max-w-md">
                    ...
                    <input type="text" placeholder="Search units..." value={searchQuery}
                    ...
                    <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                            {loading ? (
                                ...
                                <tr><td colSpan={3} className="px-6 py-10 text-center text-gray-500">No units found</td></tr>
                            ) : paginated.map(u => (
                                <tr key={u.id} className="hover:bg-indigo-50/50 dark:hover:bg-indigo-900/10 transition-colors">
                                    <td className="px-6 py-4 font-bold text-gray-900 dark:text-white">{u.name}</td>
                                    <td className="px-6 py-4 text-gray-600 dark:text-gray-300">{u.description || '—'}</td>
                                    <td className="px-6 py-4">
                                        <div className="flex justify-center gap-2">
                                            <button onClick={() => { setEditingUnit(u); setFormData({ name: u.name, description: u.description || '' }); setError(null); setShowModal(true); }}
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl w-full max-w-md shadow-2xl overflow-hidden">
                        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-indigo-50 dark:bg-indigo-900/20 rounded-t-xl">
                            <h2 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                                {editingUnit ? <Pencil className="w-5 h-5 text-indigo-600" /> : <Plus className="w-5 h-5 text-indigo-600" />}
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" required value={formData.name} onChange={e => setFormData(d => ({ ...d, name: e.target.value }))} className={inputCls} placeholder="e.g. tablet, vial, box" />
                            </div>
                            <div>
                            ...
                            <div className="flex gap-3 pt-2">
                                ...
                                <button type="submit" disabled={submitting} className="flex-1 py-2 bg-indigo-600 text-white rounded-lg text-sm font-bold shadow-md disabled:opacity-50">{submitting ? '...' : (editingUnit ? 'Update' : 'Save')}</button>
                            </div>
                        </form>
                    </div>
                </div>
