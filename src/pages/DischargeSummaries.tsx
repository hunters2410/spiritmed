...
export default function DischargeSummaries() {
    ...
    useEffect(() => {
        if (profile) {
            ...
            if (patientId) { setForm(prev => ({ ...prev, patient_id: patientId })); setShowModal(true); }
    ...
    async function loadRecords() {
        ...
        try {
            ...
            let q = supabase.from('discharge_summaries')
                .select('*, patient:patients(full_name, patient_number, gender, date_of_birth), doctor:users(full_name, specialization, qualifications, signature_url), diagnosis:diagnoses(name)', { count: 'exact' })
                    ...
                    <div key={s.id} className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 shadow-xs space-y-3">
                        ...
                        <div className="pt-2 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
                            ...
                            <div className="flex items-center space-x-1">
                                <button onClick={() => { setSelectedDoc(s); setViewMode('detailed'); setActionTrigger('none'); }} className="p-1.5 text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded transition" title="View Detail"><Eye className="w-4 h-4" /></button>
                                <button onClick={() => { setSelectedDoc(s); setViewMode('detailed'); setActionTrigger('print'); }} className="p-1.5 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded transition" title="Print Summary"><Printer className="w-4 h-4" /></button>
                                <button onClick={() => { setSelectedDoc(s); setViewMode('detailed'); setActionTrigger('download'); }} className="p-1.5 text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 rounded transition" title="Download PDF"><Download className="w-4 h-4" /></button>
                                <button onClick={() => {
                                    setSelectedDoc(s);
                                ...
                                <button onClick={() => handleDelete(s.id)} className="p-1.5 text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded transition" title="Delete"><Trash2 className="w-4 h-4" /></button>
                            </div>
                        </div>
                    </div>
                ))}
                        ...
                        <form onSubmit={handleSave} className="p-6 overflow-y-auto flex-1">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                <div>
                                    ...
                                    <RichTextEditor value={form.diagnosis_text} onChange={val => setForm({ ...form, diagnosis_text: val })} placeholder="Specific diagnosis details..." />
                                </div>
                                <div>
                                    <label className={labelCls}>Medical History</label>
                                    <RichTextEditor value={form.medical_history} onChange={val => setForm({ ...form, medical_history: val })} placeholder="Patient's medical history..." />
                                </div>
                                <div>
                                    <label className={labelCls}>Treatment Done</label>
                                    <RichTextEditor value={form.treatment_done} onChange={val => setForm({ ...form, treatment_done: val })} placeholder="Summary of treatment..." />
                                </div>
                                <div>
                                    <label className={labelCls}>Follow-up Plan</label>
                                    <RichTextEditor value={form.follow_up_plan} onChange={val => setForm({ ...form, follow_up_plan: val })} placeholder="Next steps for the patient..." />
                                </div>
                            </div>
