...
export default function MedicalCertificates() {
    ...
    async function loadRecords() {
        ...
        try {
            ...
            let q = supabase.from('medical_certificates')
                .select('*, patient:patients(full_name, patient_number, gender, date_of_birth), doctor:users(full_name, specialization, qualifications, signature_url)', { count: 'exact' })
                    ...
                    <div key={c.id} className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 shadow-xs space-y-3">
                        ...
                        <div className="pt-2 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
                            ...
                            <div className="flex items-center space-x-1">
                                <button onClick={() => { setSelectedDoc(c); setViewMode('detailed'); setActionTrigger('none'); }} className="p-1.5 text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded transition" title="View Detail"><Eye className="w-4 h-4" /></button>
                                <button onClick={() => { setSelectedDoc(c); setViewMode('detailed'); setActionTrigger('print'); }} className="p-1.5 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded transition" title="Print Certificate"><Printer className="w-4 h-4" /></button>
                                <button onClick={() => { setSelectedDoc(c); setViewMode('detailed'); setActionTrigger('download'); }} className="p-1.5 text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 rounded transition" title="Download PDF"><Download className="w-4 h-4" /></button>
                                <button onClick={() => {
                                    setSelectedDoc(c);
                                ...
                                <button onClick={() => handleDelete(c.id)} className="p-1.5 text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded transition" title="Delete"><Trash2 className="w-4 h-4" /></button>
                            </div>
                        </div>
                    </div>
                ))}
                        ...
                        <form onSubmit={handleSave} className="p-6 overflow-y-auto flex-1">
                            ...
                            <div className="space-y-4 mb-6">
                                <div>
                                    ...
                                    <RichTextEditor value={form.purpose} onChange={val => setForm({ ...form, purpose: val })} placeholder="Detailed purpose or clinical remarks..." minHeight="140px" />
                                </div>
                            </div>
