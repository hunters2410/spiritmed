...
export function InvoicePrintView({ data, branch, doctorSignature, onBack }: Props) {
    ...
    const handleDownloadPdf = async () => {
        ...
        pdf.save(`INVOICE_${data.invoice_number}_${data.patient.full_name}.pdf`);
    };
    ...
    const handleSendEmail = async () => {
        ...
        try {
            ...
            const fileName = `INV_${data.invoice_number}_${Date.now()}.pdf`;
            const filePath = `invoices/${data.id}/${fileName}`;
            ...
            const result = await emailService.sendEmail({
                ...
                subject: `Invoice ${data.invoice_number} from ${branch.name}`,
                body: `Dear ${data.patient.full_name},\n\nPlease find attached your invoice ${data.invoice_number} for services rendered at ${branch.name}.\n\nTotal Amount: $${data.total_amount.toLocaleString()}\n\nYou can also view your invoice here: ${publicUrl}\n\nThank you for choosing ${branch.name}.`,
                branchId: branch.id,
                referenceId: data.id,
        ...
        <div className="min-h-screen bg-gray-100 dark:bg-gray-900 p-4 md:p-8 flex flex-col md:flex-row gap-8 items-start justify-center">
            ...
            <div className="w-full md:w-64 space-y-3 print:hidden order-2 md:order-1">
                <button onClick={onBack} className="w-full flex items-center justify-center gap-3 bg-white hover:bg-gray-50 text-gray-700 border border-gray-200 px-4 py-2.5 rounded-lg font-bold text-xs transition shadow-sm">
                    <ArrowLeft className="w-4 h-4 text-emerald-600" /> Back to Dashboard
                </button>
                <button onClick={handlePrint} className="w-full flex items-center justify-center gap-3 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2.5 rounded-lg font-bold text-xs transition shadow-md">
                ...
                >
                    {isSending ? <Loader2 className="w-4 h-4 animate-spin text-emerald-600" /> : sendSuccess ? <CheckCircle className="w-4 h-4" /> : <Send className="w-4 h-4 text-emerald-600" />}
            ...
            <div className="flex-1 flex justify-center order-1 md:order-2">
                ...
                >
                    ...
                    <div className="flex justify-between items-start mb-12 border-b-2 border-emerald-600 pb-8">
                        <div className="flex items-center gap-6">
                            {branch.logo_url ? (
                                ...
                                <div className="w-16 h-16 bg-emerald-50 flex items-center justify-center rounded-xl text-emerald-600 font-black text-2xl border-2 border-emerald-100 italic">SM</div>
                            )}
                            <div>
                    ...
                    <div className="flex justify-between items-end mb-12 bg-gray-50 p-6 rounded-2xl">
                        <div>
                            <h2 className="text-4xl font-black text-gray-900 uppercase mb-1">Invoice</h2>
                            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest text-emerald-600">Official Tax Invoice</p>
                        </div>
                        <div className="text-right">
                            <div className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Invoice Number</div>
                            <div className="text-2xl font-black text-emerald-600 font-mono tracking-tighter">{data.invoice_number}</div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-12 mb-12 px-2">
                        <div>
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="text-xs font-bold text-emerald-600 font-mono tracking-tighter">ID: {data.patient.patient_number}</p>
                                <p className="text-[10px] text-gray-500 uppercase">{data.patient.email}</p>
                            </div>
                        </div>
                        <div className="text-right">
                            ...
                            <div className="space-y-1 text-xs">
                                <p className="flex justify-end gap-3 text-gray-500"><span className="font-bold uppercase opacity-50">Invoice Date:</span> <span className="text-gray-900 font-bold">{new Date(data.invoice_date).toLocaleDateString()}</span></p>
                                <p className="flex justify-end gap-3 text-gray-500"><span className="font-bold uppercase opacity-50">Due Date:</span> <span className="text-gray-900 font-bold">{new Date(data.due_date).toLocaleDateString()}</span></p>
                                <p className="flex justify-end gap-3 text-gray-500"><span className="font-bold uppercase opacity-50">Status:</span> <span className={`${data.status === 'paid' ? 'text-green-600' : 'text-amber-600'} font-black uppercase`}>{data.status.replace('_', ' ')}</span></p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex justify-end mb-20">
                        <div className="w-full max-w-xs space-y-3 bg-gray-50 p-6 rounded-2xl border border-gray-100">
                            <div className="flex justify-between text-xs text-gray-500 font-bold uppercase">
                                <span>Subtotal</span>
                                <span className="text-gray-900 font-mono">${(data.subtotal || data.total_amount).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                            </div>
                            <div className="flex justify-between text-xs text-gray-500 font-bold uppercase">
                                <span>Tax ({(data.tax_amount / (data.subtotal || 1) * 100).toFixed(0)}%)</span>
                                <span className="text-gray-900 font-mono">${data.tax_amount?.toLocaleString(undefined, { minimumFractionDigits: 2 }) || '0.00'}</span>
                            </div>
                            <div className="pt-3 border-t-2 border-emerald-600 flex justify-between items-center">
                                <span className="text-sm font-black text-gray-900 uppercase">Grand Total</span>
                                <span className="text-2xl font-black text-emerald-600 font-mono">${data.total_amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                            </div>

                            <div className="pt-4 space-y-2 border-t border-gray-100">
                                <div className="flex justify-between text-xs text-green-600 font-bold uppercase">
                                    <span>Paid Amount</span>
                                    <span className="font-mono">${(data.paid_amount || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                                </div>
                                <div className="flex justify-between text-xs text-amber-600 font-black uppercase bg-amber-50 p-2 rounded-lg">
                                    <span>Balance Due</span>
                                    <span className="font-mono">${(data.balance ?? data.total_amount).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-8 text-[10px]">
                        <div className="space-y-4">
                            <div>
                                <h4 className="font-black uppercase text-gray-400 mb-1 font-mono tracking-tighter">Payment Methods</h4>
                                <ul className="text-gray-500 space-y-1 font-medium">
                                    <li>Bank: [HOSPITAL BANK NAME]</li>
                                    <li>Account: [ACCOUNT NUMBER]</li>
                                    <li>Branch Code: [CODE]</li>
                                    <li>SWIFT: [SWIFT]</li>
                                </ul>
                            </div>
                        </div>
