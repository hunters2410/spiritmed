...
# Load existing patient_numbers from DB
print("Loading existing patient_numbers from DB...")
existing = set()
for offset in range(0, 10000, 1000):
    ...
    if not batch: break
    for p in batch: existing.add(str(p["patient_number"]))
    if len(batch) < 1000: break
print(f"Already in DB: {len(existing)} patients")

# Parse dump and find missing
print("Parsing dump for missing patients...")
with open(r"database 4\patient (8).sql","r",encoding="utf-8",errors="replace") as f:
    lines=f.readlines()
missing=[]; used_fn=set(); used_emails=set()
for line in lines:
    s=line.strip()
    ...
    if pid in existing: continue  # already inserted
    ...
    if email in used_emails: email=f"legacy.{pid}.{len(missing)}@spiritmed.local"
    used_emails.add(email)
    missing.append({"patient_number":pid,"full_name":name,"email":email,"phone":phone,
        ...
        "id_passport_number":id_passport,"file_number":fn,"status":status,
        "next_of_kin_name":nok_name,"next_of_kin_phone":nok_phone,
        "next_of_kin_relationship":nok_rel,"smoke":smoke,"alcohol":alcohol,
        "branch_id":BRANCH_ID,"created_at":"2023-07-11T00:00:00Z"})

print(f"Remaining to insert: {len(missing)}")

# Insert remaining in batches of 200, timeout=10s
ok=0; err=0; errors=[]
for i in range(0, len(missing), 200):
    ...
    try:
        with urllib.request.urlopen(req,timeout=10) as r: ok+=len(batch)
        ...
        for p in batch:
            ...
            try:
                with urllib.request.urlopen(req2,timeout=10) as r2: ok+=1
            except Exception as e2:
                err+=1; errors.append(f"{p['patient_number']}: {str(e2)[:60]}")
            time.sleep(0.01)
    if (i//200+1)%5==0:
        print(f"  {min(i+200,len(missing))}/{len(missing)} ({min(i+200,len(missing))/len(missing)*100:.0f}%) OK={ok} Err={err}",flush=True)
    time.sleep(0.02)
...
# Final count
req3 = urllib.request.Request(SUPABASE_URL+"/rest/v1/patients?select=id",
...
with urllib.request.urlopen(req3,timeout=15) as r3:
    total = r3.getheader("Content-Range","0/?").split("/")[-1]
