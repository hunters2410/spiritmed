...
def main():
    ...
    print(f"Bills Parsed: {len(bills_map)} (Matched Patient: {bill_matched}, Unmatched Fallback: {bill_unmatched})")
    ...
    for t in d_tuples:
        ...
        if amt <= 0:
            continue # Skip 0 amount placeholder deposits
    ...
    print(f"Active Payments/Deposits to Import: {len(payments_to_import)}")
    print(f"Deposit Patient Matching: Matched={dep_matched}, Fallback={dep_unmatched}")
    ...
    for old_pay_id, b in bills_map.items():
        ...
        if pd >= b['total_amount'] and b['total_amount'] > 0:
            ...
            b['status'] = 'partial'
            partial_bills_count += 1
        else:
            b['status'] = 'unpaid'
            unpaid_bills_count += 1

    print("\n=== RECALCULATED BILL STATUSES ===")
    print(f"  Paid Bills    : {paid_bills_count:>5d} ({(paid_bills_count/len(bills_map))*100:.1f}%)")
    print(f"  Partial Bills : {partial_bills_count:>5d} ({(partial_bills_count/len(bills_map))*100:.1f}%)")
    print(f"  Unpaid Bills  : {unpaid_bills_count:>5d} ({(unpaid_bills_count/len(bills_map))*100:.1f}%)")
